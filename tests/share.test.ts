...
test("URL round trip preserves IDs, order, duplicates, blanks and arbitrary drafts", () => {
  const entries = [{ id: 7, input: "192.168.0.7/24" }, { id: 2, input: "192.168.0.7/24" }, { id: 9, input: "" }, { id: 10, input: "編集中 % / # +" }];
  ...
  assert.deepEqual(decodeEntries("#v1=" + encodeURIComponent('[[3,"192.168.0.1/24"]]')), [{ id: 3, input: "192.168.0.1/24" }]);
...
test("malformed URLs and unsupported versions are rejected", () => {
  ...
  assert.throws(() => encodeEntries([{ id: 1, input: "a".repeat(64_000) }]));
});
